...
const TrafficCard = ({ label, value, diff, subLabel, compareValue }) => {
  ...
  const valueColor = label.includes("Duration")
  ...
  return (
    <div className="p-4 bg-gray-100 rounded-lg shadow-lg shadow-gray-200  flex flex-col">
      <h3 className="text-md text-gray-500">{label}</h3>
      {typeof diff === "number" ? (
        <p
          className={`text-sm ${diff < 0 ? "text-red-500" : "text-green-600"}`}
        >
          {diff < 0 ? "↓" : "↑"} {Math.abs(diff)}% Comparison Percentage
        </p>
      ) : (
